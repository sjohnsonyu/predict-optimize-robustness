...
def opeIS_parallel(state_record, action_record, reward_record, w, n_benefs, T, K, n_trials, gamma, target_policy_name, beh_policy_name, single_trajectory=False, epsilon=0.1):
    compare = {'target':policy_map[target_policy_name], 'beh':policy_map[beh_policy_name]}
    ...
    beh_probs_raw    = tf.reshape(getProbs(state_record[:,0,:,:].reshape(-1, N), policy=compare['beh'],    ts=None, w=w, k=K, epsilon=epsilon), (ntr, L, N))
...
class opeSimulator(object):
    def __init__(self, beh_traj, n_benefs, T, m, OPE_sim_n_trials, gamma, beh_policy_name, T_data, R_data, env='general', H=None, use_informed_prior=False, baseline_mode=None):
